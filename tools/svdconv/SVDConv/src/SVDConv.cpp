...
void SvdConv::PrintPeripheral(SvdPeripheral *const peri, const std::__cxx11::list<SvdInterrupt *> &interrupts)
{
  ...
  cout << "disableCondition: " << peri->GetDisableCondition() << endl;
  cout << "baseAddress: 0x" << hex << peri->GetAddress() << dec << endl;
  cout << "size: " << peri->GetEffectiveBitWidth() << endl;
...
void SvdConv::PrintCluster(SvdCluster *cluster, int numSpaces)
{
  ...
  cout << space << "size: " << cluster->GetEffectiveBitWidth() << endl;
...
void SvdConv::PrintRegister(SvdRegister *reg, int numSpaces)
{
  ...
  cout << space << "=== Register " << reg->GetNameCalculated() << " ===" << endl;
  ...
  cout << space << "size: " << reg->GetEffectiveBitWidth() << endl;
  ...
  cout << space << "writeConstraint: not implemented" << endl;
